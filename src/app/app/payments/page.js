  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">{payments.length} payments &middot; ${totalCompleted.toLocaleString()} completed</p>
      </div>

      <div className="flex gap-2 flex-wrap">
        <button onClick={() => { setStatusFilter(''); setLoading(true); }} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${!statusFilter ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>All</button>
        {['pending', 'completed', 'failed', 'refunded'].map(s => (
          <button key={s} onClick={() => { setStatusFilter(s); setLoading(true); }} className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize ${statusFilter === s ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{s}</button>
        ))}
      </div>
          ...
          {payments.map(p => (
            <div key={p.id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition">
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-lg ${p.status === 'completed' ? 'bg-emerald-50' : 'bg-gray-50'}`}>
                  <CreditCard className={`w-5 h-5 ${p.status === 'completed' ? 'text-emerald-600' : 'text-gray-400'}`} />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-gray-900">${parseFloat(p.amount).toLocaleString()}</span>
              ...
              <div className="text-right text-xs text-gray-400">
                {p.account_name && <div className="text-gray-600">{p.account_name}</div>}
