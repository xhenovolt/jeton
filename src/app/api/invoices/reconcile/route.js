...
import { generateInvoiceNumber } from '@/lib/invoice-engine.js';
  ...
  try {
    ...
    const candidates = await query(
      `SELECT d.id, d.title, d.value_estimate, d.currency, d.client_id, d.client_name,
              ...
              d.system_id, d.stage,
              COALESCE(
                (SELECT SUM(p.amount) FROM payments p WHERE p.deal_id = d.id AND p.status = 'completed'),
                0
              ) AS collected
       FROM deals d
    ...
    for (const d of candidates.rows) {
      try {
        // Reuse the same counter + format as the automatic per-payment
        // invoices (src/lib/invoice-engine.js). This used to duplicate the
        // invoice_sequences logic inline with a different prefix
        // ("XH-INV-" instead of "XHV-INV-"), so numbers generated here and
        // by the payment flow interleaved on the same yearly counter but
        // came out with inconsistent, non-matching prefixes.
