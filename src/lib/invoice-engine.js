...
export async function createInvoiceForPayment({ payment, deal, userId }) {
  try {
    ...
    // Surface the failure instead of letting it die in server logs only —
    // a payment that silently fails to produce an invoice looks, from the
    // app, like "nothing happened" with no way to tell why.
    dispatch('invoice_generation_failed', {
      entityType: 'payment',
      entityId: payment?.id || null,
      description: `Invoice generation failed for payment on "${deal?.title || 'deal'}": ${err.message}`,
      metadata: {
        deal_id: deal?.id,
        deal_title: deal?.title,
