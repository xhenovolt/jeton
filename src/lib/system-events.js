...
const NOTIFICATION_MAP = {
  ...
  invoice_generation_failed: {
    ...
    message: (actor, m) => `Payment${m.deal_title ? ` for "${m.deal_title}"` : ''} was recorded, but the invoice failed to generate: ${m.error || 'unknown error'}`,
  },
  payment_recording_failed: {
    type: 'warning',
    title: () => `Payment Recording Failed`,
    message: (actor, m) => `${actor} — initial payment failed while creating deal "${m.title || ''}": ${m.error || 'unknown error'}`,
  },
  bug_reported: {
    type: 'warning',
